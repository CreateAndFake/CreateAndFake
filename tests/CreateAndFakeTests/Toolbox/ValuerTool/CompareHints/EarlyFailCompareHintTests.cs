...
namespace CreateAndFakeTests.Toolbox.ValuerTool.CompareHints
{
    ...
    public sealed class EarlyFailCompareHintTests : CompareHintTestBase<EarlyFailCompareHint>
    {
        ...
        {
            Tools.Asserter.Is(true, TestInstance.TryCompare(null, new object(), Tools.Valuer).Item1);
            Tools.Asserter.IsNotEmpty(TestInstance.TryCompare(null, new object(), Tools.Valuer).Item2);

            Tools.Asserter.Is(true, TestInstance.TryCompare(null, null, Tools.Valuer).Item1);
            Tools.Asserter.IsEmpty(TestInstance.TryCompare(null, null, Tools.Valuer).Item2);

            Tools.Asserter.Is(true, TestInstance.TryCompare(new object(), null, Tools.Valuer).Item1);
            Tools.Asserter.IsNotEmpty(TestInstance.TryCompare(new object(), null, Tools.Valuer).Item2.ToArray());
        ...
        {
            Tools.Asserter.Is(true, TestInstance.TryGetHashCode(null, Tools.Valuer).Item1);
            Tools.Asserter.Is(ValueComparer.NullHash, TestInstance.TryGetHashCode(null, Tools.Valuer).Item2);
